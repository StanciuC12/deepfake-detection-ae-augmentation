...
class U_Net(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=3):
        ...
        self.Conv5 = conv_block(filters[3], filters[4], padding='same')
...
class NestedUNet(nn.Module):
    ...
    def up_cat(self, x, y_up):

        Up = nn.Upsample(size=(x.shape[-2::]), mode='bilinear', align_corners=True)
        return torch.cat([x, Up(y_up)])

    def forward(self, x):
        x0_0 = self.conv0_0(x)
        x1_0 = self.conv1_0(self.pool(x0_0))
        up_00 = nn.Upsample(size=x0_0.shape[-2::], mode='bilinear', align_corners=True)
        x0_1 = self.conv0_1(torch.cat([x0_0, up_00(x1_0)], 1))

        x2_0 = self.conv2_0(self.pool(x1_0))
        up_10 = nn.Upsample(size=x1_0.shape[-2::], mode='bilinear', align_corners=True)
        x1_1 = self.conv1_1(torch.cat([x1_0, up_10(x2_0)], 1))
        x0_2 = self.conv0_2(torch.cat([x0_0, x0_1, up_00(x1_1)], 1))

        x3_0 = self.conv3_0(self.pool(x2_0))
        up_20 = nn.Upsample(size=x2_0.shape[-2::], mode='bilinear', align_corners=True)
        x2_1 = self.conv2_1(torch.cat([x2_0, up_20(x3_0)], 1))
        x1_2 = self.conv1_2(torch.cat([x1_0, x1_1, up_10(x2_1)], 1))
        x0_3 = self.conv0_3(torch.cat([x0_0, x0_1, x0_2, up_00(x1_2)], 1))

        x4_0 = self.conv4_0(self.pool(x3_0))
        up_30 = nn.Upsample(size=x3_0.shape[-2::], mode='bilinear', align_corners=True)
        x3_1 = self.conv3_1(torch.cat([x3_0, up_30(x4_0)], 1))
        x2_2 = self.conv2_2(torch.cat([x2_0, x2_1, up_20(x3_1)], 1))
        x1_3 = self.conv1_3(torch.cat([x1_0, x1_1, x1_2, up_10(x2_2)], 1))
        x0_4 = self.conv0_4(torch.cat([x0_0, x0_1, x0_2, x0_3, up_00(x1_3)], 1))
...
class ExpansiveBlock(nn.Module):
    ...
    def __init__(self, in_filters1, in_filters2, out_filters, tr_kern=3, conv_kern=3, stride=2, dropout=0.5):
        ...
        self.t2 = nn.ConvTranspose2d(in_filters1, out_filters, tr_kern, stride=2, padding=1, output_padding=0)
        self.d1 = nn.Dropout(dropout)
        self.c1 = ConvolutionBlock(out_filters + in_filters2, out_filters, conv_kern)

    def forward(self, x, contractive_x):
        x_ups = self.t1(x)
        if x_ups.shape[-2::] != contractive_x.shape[-2::]:
            x_ups = self.t2(x)
