...
class DeepFakeDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root_dir, train_file, batch_size=1, transform=None,
                 train=True, image_type='fullface', dataset='all', frames=20, skip=1, image_dim=(3, 299, 299)):
        ...
        for folder in folders:
            ...
            if not empty_folder:
                ...
                for img_name in images:
                    ...
                    if self.image_dim == (3, 299, 299):
                        X = np.array(Image.open(img), dtype=np.float32)
                    else:
                        X = np.array(Image.open(img).resize(self.image_dim), dtype=np.float32)
                    if np.max(X) > 1:
                        X = X / 255
